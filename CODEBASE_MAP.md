...
Last reviewed: 2026-08-05. This is the compact, implementation-light entry point for future work. Read this before opening source files; update it whenever routes, packages, persistence models, or deployment boundaries change.
...
| Path | Responsibility |
| --- | --- |
| `README.md` | Setup, commands, development rules, mobile workflow, and current priorities. |
| `AGENTS.md` | Safe contribution workflow and handoff checklist for agents/developers. |
| `docs/RELEASE_CUTOVER.md` | Ordered production database, deployment, lock, and smoke-test procedure. |
...
| `scripts/` | OCR startup, database import, and post-merge development utilities. |
...
Provider hierarchy is Query Client → tooltips/confirmation → Auth → Session → router. `GlobalDataProvider` is mounted by the student page and preloads personal programs/subjects, logic nodes/progress, Chrono state/inventory, and lobby state. Classroom data is loaded by the new `classroomService` document model when the Classes view opens. Cross-feature navigation/state also uses `ll:*` browser events; the main contracts are `ll:setView`, `ll:openClassContent`, `ll:setPendingSession`, `ll:setOngoingWarmup`, `ll:personalProgramCreated`, `ll:personalProgramDeleted`, `ll:subjectsUpdated`, and `ll:openMyPrograms`.

Frontend domain logic lives in `artifacts/web-app/src/lib/*Service.ts` (and nearby helpers), grouped broadly as:

- Identity/roles: auth, users, admin, teacher, parent, student, linking.
...
| `POST /api/auth/password-login` | Rate-limited email/username login without exposing profile email |
| `POST /api/auth/username-available` | Privacy-safe username availability check |
| `POST /api/admin/users/{create,delete,economy,impersonation-token}` | Audited server-side managed-account operations |
| `POST /api/account/create-linked-parent` | Create and link a parent account server-side |

Each API feature follows routes → validation/controller → service → provider(s). Program ingestion additionally has a repository, file store, in-process job queue, pipeline/stage providers, segmentation/normalization/anomaly logic, and Python PDF extraction. Job files default to `artifacts/api-server/.data/program-ingestion`; this ignored runtime scratch space is ephemeral unless the host mounts durable storage.
...
Public RPC surface used by the SPA includes the hardened friend functions and narrowly scoped classroom/guardian actions. Managed accounts, role changes, wallet authority, and impersonation are server-side operations. RLS helper functions centralize role, new-classroom membership, assigned-admin, teacher, and parent/student checks. General program files use `program-assets`; homework and submissions use the private `classroom-homework` bucket with short-lived signed URLs.
...
Frontend environment: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_API_SERVER_URL`, `VITE_BASE_PATH`, `VITE_SENTRY_DSN`, `VITE_APP_VERSION`, and `VITE_OCR_SERVER_URL`. Never configure service-role or provider secrets with a `VITE_` prefix.

API/database environment: `DATABASE_URL`, `PORT` (default 5000), `NODE_ENV`, `CORS_ALLOWED_ORIGINS`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `LOG_LEVEL`, `SENTRY_DSN`, `APP_VERSION`, AI provider keys, provider/model switches prefixed `PROGRAM_INGESTION_` and `FREEFORM_GRADING_`, and OCR switches prefixed `HANDWRITING_OCR_` / `SYMBOL_RECOGNITION_`.

`vercel.json` pins the pnpm install command, and `api/index.ts` is the Vercel-facing API entry. Confirm the Vercel project dashboard's root, build/output, API routing, and environment settings because those remain partly external configuration. GitHub Pages builds only the SPA through `.github/workflows/deploy.yml`. The SPA build copies `index.html` to `404.html` for history fallback. Capacitor builds use the same `web-app/dist` output.
...
- Treat `artifacts/web-app/src/` and `artifacts/api-server/src/` as runtime source. Generated output, dependencies, logs, local environments, API `.data`, and backups are ignored and must not be committed.
...
| Role/admin/class feature | `TeacherPage.tsx`, `UnifiedRolePages.tsx`, classroom components, `classroomService`, `adminService`, `teacherService`, `parentService`, `studentService` |
...
| Deployment | `docs/RELEASE_CUTOVER.md`, root `vercel.json`, `api/index.ts`, package build scripts, `web-app/vite.config.ts`, API `build.mjs`, and external host settings |
