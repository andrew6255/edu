...
function StudentWork({ homework, studentId, expired }: { homework: Homework; studentId: string; expired: boolean }) {
  ...
  return <>
    ...
    <Section title="Uploaded files" empty="No files uploaded yet.">{files.map(file => <WorkCard key={file.id} icon="📄" title={file.name} subtitle={`Uploaded ${new Date(file.uploadedAt).toLocaleString()}`} onOpen={() => window.open(file.url, '_blank')} actions={!expired ? <button onClick={() => { if (!window.confirm(`Delete “${file.name}”?`)) return; setSaving(true); void deleteHomeworkAttachment(file.path).then(() => changeFiles(files.filter(item => item.id !== file.id))).catch(reason => setError(reason instanceof Error ? reason.message : 'Delete failed.')).finally(() => setSaving(false)); }} style={{ ...button, color: '#f87171' }}>Delete</button> : undefined} />)}</Section>
