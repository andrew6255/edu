...
export default function ProgramMapView({ onBack, programId: programIdProp }: { onBack: () => void; programId?: string | null }) {
  ...
  const canGoSoloBack = soloHistoryIndex > 0;
  const canGoSoloForward = soloHistoryIndex >= 0 && soloHistoryIndex < soloHistoryIds.length - 1;

  function resetSoloResponseState() {
    setSoloFeedback(null);
    setSoloTextAnswer('');
    setSoloStepAnswers({});
    setSoloShowExplanation(false);
  ...
  function openSoloQuestion(questionId: string, options?: { appendToHistory?: boolean }) {
    setSoloQuestionId(questionId);
    resetSoloResponseState();
    if (options?.appendToHistory === false) return;
    setSoloHistoryIds((prev) => {
      const truncated = soloHistoryIndex >= 0 ? prev.slice(0, soloHistoryIndex + 1) : prev;
      if (truncated[truncated.length - 1] === questionId) {
        setSoloHistoryIndex(truncated.length - 1);
        return truncated;
      }
      const nextHistory = [...truncated, questionId];
      setSoloHistoryIndex(nextHistory.length - 1);
      return nextHistory;
    });
  }
  ...
  function nextSolo() {
    ...
    if (canGoSoloForward) {
      const nextIndex = soloHistoryIndex + 1;
      const nextId = soloHistoryIds[nextIndex] ?? null;
      if (nextId) {
        setSoloHistoryIndex(nextIndex);
        setSoloQuestionId(nextId);
        resetSoloResponseState();
      }
      return;
    }
  ...
  function previousSolo() {
    if (!canGoSoloBack) return;
    const prevIndex = soloHistoryIndex - 1;
    const prevId = soloHistoryIds[prevIndex] ?? null;
    if (!prevId) return;
    setSoloHistoryIndex(prevIndex);
    setSoloQuestionId(prevId);
    resetSoloResponseState();
  }
                      ...
                      {!soloCurrent ? (
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
                            <button
                              onClick={previousSolo}
                              disabled={!canGoSoloBack}
                              className="ll-btn"
                              style={{ padding: '8px 12px', fontSize: 12, minWidth: 108, opacity: canGoSoloBack ? 1 : 0.55 }}
                            >
                              ← Back
                            </button>
                            <button
                              onClick={nextSolo}
                              disabled={soloAwarding || (!soloCurrent && !canGoSoloForward)}
                              className="ll-btn"
                              style={{ padding: '8px 12px', fontSize: 12, minWidth: 108, opacity: soloAwarding || (!soloCurrent && !canGoSoloForward) ? 0.55 : 1 }}
                            >
                              Next →
                            </button>
                          </div>
