    ...
    try {
      ...
      const sorted = users
        ...
        .map(u => ({
          ...
          wins: u.arenaStats?.wins ?? 0,
          xp: u.economy?.global_xp || 0,
          streak: u.arenaStats?.highestStreak ?? 0
        }))
  ...
  if (screen === 'result' && lastResult) {
    ...
    return (
      <div style={{
        ...
        background: won
          ? 'radial-gradient(circle, rgba(251,191,36,0.09) 0%, #0f172a 70%)'
          : 'radial-gradient(circle, rgba(239,68,68,0.09) 0%, #0f172a 70%)',
        animation: 'fadeIn 0.5s ease'
      }}>
        <div style={{ fontSize: 88 }}>{won ? '🏆' : '💀'}</div>
        <h1 style={{
          margin: 0, fontSize: 38, letterSpacing: 4, fontWeight: 'black',
        ...
        <div style={{
          display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, width: '100%', maxWidth: 360
        }}>
          {[
            ...
            <div key={s.label} style={{
              background: '#1e293b', borderRadius: 12, padding: '12px', textAlign: 'center',
        ...
        {stats.highestStreak >= 3 && (
          <div style={{
            background: 'rgba(251,191,36,0.1)', borderRadius: 10, padding: '10px 20px',
            border: '1px solid rgba(251,191,36,0.3)', textAlign: 'center'
          }}>
            <span style={{ color: '#fbbf24', fontWeight: 'bold', fontSize: 15 }}>
        ...
        <div style={{
          background: '#1e293b', borderRadius: 12, padding: '14px 22px',
          border: '1px solid #334155', textAlign: 'center', width: '100%', maxWidth: 360
        }}>
          ...
          <div style={{ color: '#64748b', fontSize: 12 }}>
            Arena record: <strong style={{ color: 'white' }}>{arenaStats.wins}W / {arenaStats.losses}L</strong>
          </div>
        </div>
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '14px 14px 20px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 16 }}>
        ...
        <h2 style={{ margin: 0, fontSize: 22, fontWeight: 'bold', color: 'white', letterSpacing: 2 }}>BATTLE ARENA</h2>
        <p style={{ color: '#64748b', fontSize: 12, margin: '4px 0 0' }}>Answer questions to defeat your enemies</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 18 }}>
        {[
          ...
          { label: 'Best Streak', value: arenaStats.highestStreak || '—', icon: '🔥', color: '#f97316' },
        ].map(s => (
          <div key={s.label} style={{
            background: '#1e293b', borderRadius: 10, padding: '10px 6px',
        ...
        {ENEMIES.map(enemy => {
          ...
          return (
            <div key={enemy.id} style={{
              background: '#1e293b', borderRadius: 14, padding: '14px',
              border: `1px solid ${isLocked ? '#334155' : enemy.color + '44'}`,
              opacity: isLocked ? 0.6 : 1,
              boxShadow: isLocked ? 'none' : `0 0 14px ${enemy.color}11`
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ fontSize: 48, flexShrink: 0, filter: isLocked ? 'grayscale(1)' : 'none' }}>
                  {isLocked ? '🔒' : enemy.avatar}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 2, flexWrap: 'wrap' }}>
                    <span style={{ fontWeight: 'bold', color: 'white', fontSize: 15 }}>{enemy.name}</span>
                    <span style={{
                      fontSize: 9, padding: '2px 7px', borderRadius: 4, fontWeight: 'bold',
                      background: `${diff.color}22`, border: `1px solid ${diff.color}55`, color: diff.color
                    }}>
                      {diff.label}
                    </span>
                  </div>
                  <div style={{ color: '#94a3b8', fontSize: 11, marginBottom: 4 }}>{enemy.title}</div>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ color: '#10b981', fontSize: 12, fontWeight: 'bold', marginBottom: 3 }}>+{enemy.xpReward} XP</div>
      ...
      <div style={{ background: '#1e293b', borderRadius: 14, border: '1px solid #334155', overflow: 'hidden', marginBottom: 8 }}>
        {loadingLb ? (
          <div style={{ color: '#64748b', padding: '18px', textAlign: 'center', fontSize: 13 }}>Loading...</div>
        ) : leaderboard.length === 0 ? (
          <div style={{ color: '#64748b', padding: '18px', textAlign: 'center', fontSize: 13 }}>No battles yet. Be the first!</div>
        ) : (
          leaderboard.map((p, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: 15, width: 26, textAlign: 'center', flexShrink: 0 }}>{medal}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ fontWeight: isMe ? 'bold' : 'normal', color: isMe ? '#93c5fd' : 'white', fontSize: 13 }}>
                    {p.username}{isMe ? ' (you)' : ''}
                  </span>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ color: '#fbbf24', fontWeight: 'bold', fontSize: 13 }}>{p.wins}W</div>
                  {p.streak >= 3 && (
                    <div style={{ color: '#f97316', fontSize: 10 }}>🔥{p.streak}</div>
                  )}
                </div>
              </div>
            );
