...
function ClassList({ rows, open }: { rows: TeacherClass[]; open: (row: TeacherClass) => void }) { return <div style={{ display: 'grid', gap: 8 }}>{rows.length ? rows.map(row => <button key={row.id} onClick={() => open(row)} style={{ ...card, color: 'white', textAlign: 'left', cursor: 'pointer' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><strong>{row.name}</strong><span style={{ textAlign: 'right' }}>{row.subjectEmoji || '📘'} {row.subject}</span></div><small style={{ color: '#64748b' }}>{row.teacherName} · {row.status}</small></button>) : <Empty text="No classrooms." />}</div>; }
...
function ParentClass({ cls, child, back }: { cls: TeacherClass; child: LinkedStudent; back: () => void }) {
  ...
  return <div><button onClick={back} style={btn}>← Classrooms</button><h2>{cls.name}</h2><p>{cls.subjectEmoji || '📘'} {cls.subject} · {cls.teacherName}</p><h3>Sessions</h3>{sessions.length ? sessions.map(s => <div key={s.id} style={{ ...card, marginBottom: 7 }}><strong>{s.name}</strong><div style={{ color: '#94a3b8', fontSize: 12 }}>{new Date(s.date).toLocaleString()} · {s.status}</div></div>) : <Empty text="No sessions." />}<h3>Homework</h3>{homeworks.length ? homeworks.map(h => { const s = work[h.id]; return <div key={h.id} style={{ ...card, marginBottom: 7 }}><strong>{h.title}</strong>{h.documents.map(document => <a key={document.id} href={document.url} target="_blank" rel="noreferrer" style={{ display: 'block', color: '#93c5fd', fontSize: 12, marginTop: 4 }}>📕 {document.name}</a>)}<div style={{ fontSize: 12, color: '#94a3b8' }}>Due {new Date(h.dueAt).toLocaleString()}</div><div style={{ fontSize: 12 }}>{s ? s.sheets.length + ' sheets · ' + s.attachments.length + ' files' : 'Not started'}</div>{s?.attachments.map(f => <a key={f.id} href={f.url} target="_blank" rel="noreferrer" style={{ display: 'block', color: '#c4b5fd' }}>📄 {f.name}</a>)}</div>; }) : <Empty text="No homework." />}</div>;
}

function ParentProgress({ classes, child }: { classes: TeacherClass[]; child: LinkedStudent }) {
  const [rows, setRows] = useState<Array<{ cls: TeacherClass; total: number; started: number; sheets: number; files: number }>>([]);
  useEffect(() => { void Promise.all(classes.map(async cls => {
    const homeworks = await listClassHomeworks(cls.id);
    const work = await Promise.all(homeworks.map(homework => getHomeworkSubmission(homework.id, child.id)));
    return { cls, total: homeworks.length, started: work.filter(Boolean).length, sheets: work.reduce((sum, item) => sum + (item?.sheets.length ?? 0), 0), files: work.reduce((sum, item) => sum + (item?.attachments.length ?? 0), 0) };
  })).then(setRows); }, [classes, child.id]);
  return <div style={{ display: 'grid', gap: 8 }}>{rows.length ? rows.map(row => <div key={row.cls.id} style={card}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}><strong>{row.cls.name}</strong><span>{row.cls.subjectEmoji || '📘'} {row.cls.subject}</span></div><div style={{ color: '#94a3b8', fontSize: 12, marginTop: 7 }}>{row.started}/{row.total} homework items started · {row.sheets} sheets · {row.files} files</div></div>) : <Empty text="No classroom progress yet." />}</div>;
}
...
export function UnifiedParentPage() {
  ...
  return <Shell title="👨‍👩‍👧 Parent" color="#ec4899" tabs={tabs} active={tab} onTab={id => { setTab(id); setSelected(null); }}>{pendingConsents.length > 0 && <div style={{ ...card, borderColor: '#f59e0b', marginBottom: 12 }}><strong>Guardian approval requests</strong>{pendingConsents.map(request => <div key={request.consentId} className="phone-wrap" style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}><span style={{ flex: 1 }}>{request.studentUsername}<small style={{ display: 'block', color: '#94a3b8' }}>Requested {new Date(request.requestedAt).toLocaleDateString()}</small></span><button style={{ ...btn, color: '#86efac' }} onClick={async () => { await decideGuardianConsent(request.consentId, true); await loadFamily(); }}>Approve</button><button style={{ ...btn, color: '#fca5a5' }} onClick={async () => { await decideGuardianConsent(request.consentId, false); await loadFamily(); }}>Decline</button></div>)}</div>}<select value={child?.id || ''} onChange={e => setChild(children.find(c => c.id === e.target.value) || null)} style={{ ...btn, marginBottom: 12 }}>{children.map(c => <option key={c.id} value={c.id}>{c.username}</option>)}</select>{!child ? <Empty text="Link a student account to begin." /> : selected ? <ParentClass cls={selected} child={child} back={() => setSelected(null)} /> : tab === 'classrooms' ? <ClassList rows={classes} open={setSelected} /> : tab === 'overview' ? <div style={card}><h3>{child.username}</h3><p>{classes.length} classrooms. Open Classrooms to see homework and live work.</p></div> : tab === 'progress' ? <ParentProgress classes={classes} child={child} /> : <Empty text="Classroom messaging will appear here when a teacher starts a conversation." />}</Shell>;
}

function TAClass({ cls, back }: { cls: TeacherClass; back: () => void }) {
  const [members, setMembers] = useState<ClassMember[]>([]);
  const [homeworks, setHomeworks] = useState<Homework[]>([]);
  const [drafts, setDrafts] = useState<Record<string, HomeworkSubmission[]>>({});
  useEffect(() => { void Promise.all([getClassMembers(cls.id), listClassHomeworks(cls.id)]).then(async ([nextMembers, nextHomeworks]) => {
    setMembers(nextMembers.filter(member => !member.kickedAt)); setHomeworks(nextHomeworks);
    setDrafts(Object.fromEntries(await Promise.all(nextHomeworks.map(async homework => [homework.id, await listHomeworkSubmissions(homework.id)] as const))));
  }); }, [cls.id]);
  const nameFor = (studentId: string) => members.find(member => member.userId === studentId)?.fullName || studentId;
  return <div><button onClick={back} style={btn}>← Classrooms</button><h2>{cls.name}</h2><p>{cls.subjectEmoji || '📘'} {cls.subject} · {members.length} participants</p><h3>Homework and student work</h3>{homeworks.length ? homeworks.map(homework => <div key={homework.id} style={{ ...card, marginBottom: 8 }}><strong>{homework.title}</strong><small style={{ display: 'block', color: '#94a3b8', marginTop: 3 }}>Due {new Date(homework.dueAt).toLocaleString()}</small>{(drafts[homework.id] || []).map(work => <div key={work.id} style={{ marginTop: 6, fontSize: 12 }}>{nameFor(work.studentId)} · {work.sheets.length} sheets · {work.attachments.length} files · updated {new Date(work.updatedAt).toLocaleString()}</div>)}</div>) : <Empty text="No homework in this classroom." />}</div>;
}

export function UnifiedTAPage() {
  const { user, userData, loading } = useAuth(); const [, go] = useLocation(); const [tab, setTab] = useState('classrooms'); const [classes, setClasses] = useState<TeacherClass[]>([]); const [selected, setSelected] = useState<TeacherClass | null>(null);
  ...
  return <Shell title="✏️ TA" color="#06b6d4" tabs={[{ id: 'classrooms', icon: '🏫', label: 'Classrooms' }, { id: 'parents', icon: '👨‍👩‍👧', label: 'Parent Reports' }]} active={tab} onTab={id => { setTab(id); setSelected(null); }}>{selected ? <TAClass cls={selected} back={() => setSelected(null)} /> : tab === 'classrooms' ? <ClassList rows={classes} open={setSelected} /> : <Empty text="Parent reports will appear here when a teacher shares one with you." />}</Shell>;
}
